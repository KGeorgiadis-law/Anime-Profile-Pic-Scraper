...
# 3. Use bs4 to create a list of all the links * (idea, how about using a
...
# 4. It should be easy to scrape the name from either the original character page or the character page.
# Then use the os module to create a new folder with the id and the name.
# 5. Cycle through the set created in step 3. and download the pictures.
# A good option to do this is `urllib.request.urlretrieve()` (<a href='https://stackoverflow.com/a/8286449'>see here
# for reference</a>)
...
def create_dicts_from_character_id(character_id):

    # ## Step 1:
    # * Cycle through all the characters in https://myanimelist.net starting from https://myanimelist.net/character/1
    # * Character IDs are incremental - we can simply start from https://myanimelist.net/character/1 and then
    # add 1 until we receive an error.
    # * As a fail-safe, **consider saving the enumerator in an outside text file**.
    ...
    # create a link
    character_page_url = "https://myanimelist.net/character/"+character_id+"/"
    ...
    picture_urls = set()
    for picture in picture_tags_set:
